...
2 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
| | PART | Particle: a function word that must be associated with an- other word | 's, not, (infinitive) to |
...
17.1 • (MOSTLY) ENGLISH WORD CLASSES 3
...
4 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
17.2 • PART-OF-SPEECH TAGGING 5

[IMAGE: Diagram illustrating the task of part-of-speech tagging: Input words ($x_1$-$x_5$: Janet, will, back, the, bill) mapping to output POS tags ($y_1$-$y_5$: NOUN, AUX, VERB, DET, NOUN) via a Part of Speech Tagger.]
...
6 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
Here we re-introduce the concept of a **named entity**, which was also introduced in Section ?? for readers who haven't yet read Chapter 11.
...
17.3 • NAMED ENTITIES AND NAMED ENTITY TAGGING 7
...
8 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
[IMAGE: Figure 17.8 shows two Markov chains. (a) Weather states (HOT1, COLD2, WARM3) with transition probabilities (.6, .3, .1, etc.). (b) Word states (are, uniformly, charming) with transition probabilities (.4, .5, .1, etc.).]
...
**Markov Assumption:** $P(q_i = a|q_1 \dots q_{i-1}) = P(q_i = a|q_{i-1})$ (17.3)
...
17.4 • HMM PART-OF-SPEECH TAGGING 9

Formally, a Markov chain is specified by the following components:

| $Q = q_1q_2 \dots q_N$ | a set of $N$ states |
| :--- | :--- |
| $A = a_{11}a_{12} \dots a_{N1} \dots a_{NN}$ | a **transition probability matrix** $A$, each $a_{ij}$ represent- ing the probability of moving from state $i$ to state $j$, s.t. $\sum_{j=1}^n a_{ij} = 1 \ \forall i$ |
...
| $B = b_j(o_t)$ | a sequence of **observation likelihoods**, also called **emission probabilities**, each expressing the probability of an observation $o_t$ (drawn from a vocabulary $V = v_1, v_2, \dots, v_V$) being generated from a state $q_j$ |
| $\pi = \pi_1, \pi_2, \dots, \pi_N$ | an **initial probability distribution** over states. $\pi_i$ is the probability that the Markov chain will start in state $i$. Some states $j$ may have $\pi_j = 0$, meaning that they cannot be initial states. Also, $\sum_{i=1}^N \pi_i = 1$ |
...
**Markov Assumption:** $P(q_i|q_1, \dots, q_{i-1}) = P(q_i|q_{i-1})$ (17.6)
...
**Output Independence:** $P(o_i|q_1, \dots q_i, \dots, q_T, o_1, \dots, o_i, \dots, o_T) = P(o_i|q_i)$ (17.7)

==Start of OCR for page 10==
10 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
$$P(t_i|t_{i-1}) = \frac{C(t_{i-1}, t_i)}{C(t_{i-1})} \tag{17.8}$$

In the WSJ corpus, for example, MD occurs 13124 times of which it is followed by VB 10471, for an MLE estimate of

$$P(\text{VB}|\text{MD}) = \frac{C(\text{MD}, \text{VB})}{C(\text{MD})} = \frac{10471}{13124} \approx .80 \tag{17.9}$$
...
$$P(w_i|t_i) = \frac{C(t_i, w_i)}{C(t_i)} \tag{17.10}$$

Of the 13124 occurrences of MD in the WSJ corpus, it is associated with *will* 4046 times:

$$P(\text{will}|\text{MD}) = \frac{C(\text{MD}, \text{will})}{C(\text{MD})} = \frac{4046}{13124} \approx .31 \tag{17.11}$$

We saw this kind of Bayesian modeling in Chapter 4; recall that this likelihood term is not asking “which is the most likely tag for the word *will*?” That would be the posterior $P(\text{MD}|\text{will})$. Instead, $P(\text{will}|\text{MD})$ answers the slightly counterintuitive question "If we were going to generate a MD, how likely is it that this modal would be *will*?"

[IMAGE: Figure 17.9 illustrates two parts of an HMM representation: the A transition probabilities ($a_{12}, a_{21}, a_{23}$, etc.) and the B observation likelihoods ($B_1, B_2, B_3$) associated with states VB, MD, and NN, respectively. Example B probabilities are shown, such as P("aardvark" | MD). States VB, MD, and NN are connected by transition arcs $a_{ij}$.]
...
17.4 • HMM PART-OF-SPEECH TAGGING 11
...
$$\hat{t}_{1:n} = \arg\max_{t_{1 \dots t_n}} P(t_{1 \dots t_n}|w_{1 \dots w_n}) \tag{17.12}$$

The way we'll do this in the HMM is to use Bayes' rule to instead compute:

$$\hat{t}_{1:n} = \arg\max_{t_{1 \dots t_n}} \frac{P(w_{1 \dots w_n}|t_{1 \dots t_n})P(t_{1 \dots t_n})}{P(w_{1 \dots w_n})} \tag{17.13}$$

Furthermore, we simplify Eq. 17.13 by dropping the denominator $P(w_{1:n})$:

$$\hat{t}_{1:n} = \arg\max_{t_{1 \dots t_n}} P(w_{1 \dots w_n}|t_{1 \dots t_n})P(t_{1 \dots t_n}) \tag{17.14}$$
...
$$P(w_{1 \dots w_n}|t_{1 \dots t_n}) \approx \prod_{i=1}^n P(w_i|t_i) \tag{17.15}$$
...
$$P(t_{1 \dots t_n}) \approx \prod_{i=1}^n P(t_i|t_{i-1}) \tag{17.16}$$
...
$$\hat{t}_{1:n} = \arg\max_{t_{1 \dots t_n}} P(t_1 \dots t_n|w_1 \dots w_n) \approx \arg\max_{t_{1 \dots t_n}} \prod_{i=1}^n P(w_i|t_i) P(t_i|t_{i-1}) \tag{17.17}$$
...
12 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
$$v_t(j) = \max_{q_1 \dots q_{t-1}} P(q_1 \dots q_{t-1}, o_1, o_2 \dots o_t, q_t = j|\lambda) \tag{17.18}$$
...
$$v_t(j) = \max_{i=1}^N v_{t-1}(i) a_{ij} b_j(o_t) \tag{17.19}$$
...
17.4 • HMM PART-OF-SPEECH TAGGING 13

[IMAGE: Figure 17.11 is a sketch of the Viterbi lattice for the sentence "Janet will back the bill," showing possible tag states (DT, RB, NN, JJ, VB, MD, NNP) for each word, and highlighting the maximum likelihood path (NNP MD VB DT NN). States with zero probability of generating the word are greyed out.]
...
**Figure 17.12** The $A$ transition probabilities $P(t_i|t_{i-1})$ computed from the WSJ corpus without smoothing. Rows are labeled with the conditioning event; thus $P(\text{VB}|\text{MD})$ is 0.7968. $\langle s \rangle$ is the start token.

Let the HMM be defined by the two tables in Fig. 17.12 and Fig. 17.13. Figure 17.12 lists the $a_{ij}$ probabilities for transitioning between the hidden states (part-of-speech tags). Figure 17.13 expresses the $b_j(o_t)$ probabilities, the **observation likelihoods** of words given tags. This table is (slightly simplified) from counts in the WSJ corpus. So the word *Janet* only appears as an NNP, *back* has 4 possible parts

==Start of OCR for page 14==
14 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
[IMAGE: Figure 17.14 shows the first two columns of the Viterbi algorithm lattice for "Janet will back the bill". Cells contain the path probability and a backpointer. Calculations for states DT, RB, NN, JJ, VB, MD, NNP are shown for the words Janet (o1) and will (o2). The best path through the first two steps points from NNP (Janet) to MD (will).]
...
There are $N=7$ state columns. We begin in column 1 (for the word *Janet*) by setting the Viterbi value in each cell to the product of the $\pi$ transition probability (the start probability for that state $i$, which we get from the $\langle s \rangle$ entry of Fig. 17.12), and

==Start of OCR for page 15==
17.5 • CONDITIONAL RANDOM FIELDS (CRFS) 15
...
$$= \arg\max_{Y} \prod p(x_i|y_i) \prod p(y_i|y_{i-1}) \tag{17.21}$$

In a CRF, by contrast, we compute the posterior $p(Y|X)$ directly, training the CRF

==Start of OCR for page 16==
16 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES

to discriminate among the possible tag sequences:

$$\hat{Y} = \arg\max_{Y \in \mathcal{Y}} P(Y|X) \tag{17.22}$$
...
Let's introduce the CRF more formally, again using $X$ and $Y$ as the input and output sequences. A CRF is a log-linear model that assigns a probability to an entire output (tag) sequence $Y$, out of all possible sequences $\mathcal{Y}$, given the entire input (word) sequence $X$. We can think of a CRF as like a giant sequential version of the multinomial logistic regression algorithm we saw for text categorization. Recall that we introduced the feature function $f$ in regular multinomial logistic regression for text categorization as a function of a tuple: the input text $x$ and a single class $y$ (page ??). In a CRF, we're dealing with a sequence, so the function $F$ maps an entire input sequence $X$ and an entire output sequence $Y$ to a feature vector. Let's assume we have $K$ features, with a weight $w_k$ for each feature $F_k$:

$$p(Y|X) = \frac{\exp \left( \sum_{k=1}^K w_k F_k(X,Y) \right)}{\sum_{Y' \in \mathcal{Y}} \exp \left( \sum_{k=1}^K w_k F_k(X,Y') \right)} \tag{17.23}$$

It's common to also describe the same equation by pulling out the denominator into a function $Z(X)$:

$$p(Y|X) = \frac{1}{Z(X)} \exp \left( \sum_{k=1}^K w_k F_k(X,Y) \right) \tag{17.24}$$

$$Z(X) = \sum_{Y' \in \mathcal{Y}} \exp \left( \sum_{k=1}^K w_k F_k(X,Y') \right) \tag{17.25}$$
...
$$F_k(X,Y) = \sum_{i=1}^n f_k(y_{i-1}, y_i, X, i) \tag{17.26}$$
...
17.5 • CONDITIONAL RANDOM FIELDS (CRFS) 17
...
18 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
17.5 • CONDITIONAL RANDOM FIELDS (CRFS) 19

$$\text{prefix}(x_i) = L$$
$$\text{prefix}(x_i) = L'$$
$$\text{prefix}(x_i) = \text{L'O}$$
$$\text{prefix}(x_i) = \text{L'Oc}$$
$$\text{word-shape}(x_i) = \text{X'Xxxxxxxx}$$
$$\text{suffix}(x_i) = \text{tane}$$
$$\text{suffix}(x_i) = \text{ane}$$
$$\text{suffix}(x_i) = \text{ne}$$
$$\text{suffix}(x_i) = \text{e}$$
$$\text{short-word-shape}(x_i) = \text{X'Xx}$$
...
**Figure 17.16** Some NER features for a sample sentence, assuming that *Chicago* and *Villanueva* are listed as locations in a gazetteer. We assume features only take on the values 0 or 1, so the first POS feature, for example, would be represented as $1\{\text{POS} = \text{NNP}\}$.
...
$$= \arg\max_{Y \in \mathcal{Y}} \frac{1}{Z(X)} \exp \left( \sum_{k=1}^K w_k F_k(X,Y) \right) \tag{17.27}$$
$$= \arg\max_{Y \in \mathcal{Y}} \exp \left( \sum_{k=1}^K w_k \sum_{i=1}^n f_k(y_{i-1}, y_i, X, i) \right) \tag{17.28}$$
$$= \arg\max_{Y \in \mathcal{Y}} \sum_{k=1}^K w_k \sum_{i=1}^n f_k(y_{i-1}, y_i, X, i) \tag{17.29}$$
$$= \arg\max_{Y \in \mathcal{Y}} \sum_{i=1}^n \sum_{k=1}^K w_k f_k(y_{i-1}, y_i, X, i) \tag{17.30}$$
...
20 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
$$v_t(j) = \max_{i=1}^N v_{t-1}(i) a_{ij} b_j(o_t); \ 1 \leq j \leq N, 1 < t \leq T \tag{17.31}$$

which is the HMM implementation of

$$v_t(j) = \max_{i=1}^N v_{t-1}(i) P(s_j|s_i) P(o_t|s_j) \ 1 \leq j \leq N, 1 < t \leq T \tag{17.32}$$
...
$$v_t(j) = \max_{i=1}^N v_{t-1}(i) \prod_{k=1}^K w_k f_k(y_{i-1}, y_i, X, t) \ 1 \leq j \leq N, 1 < t \leq T \tag{17.33}$$
...
Part-of-speech taggers are evaluated by the standard metric of **accuracy**. Named entity recognizers are evaluated by **recall, precision**, and **F$_{1}$ measure**. Recall that recall is the ratio of the number of correctly labeled responses to the total that should have been labeled; precision is the ratio of the number of correctly labeled responses to the total labeled; and F-measure is the harmonic mean of the two.

To know if the difference between the F$_{1}$ scores of two NER systems is a significant difference, we use the paired bootstrap test, or the similar randomization test (Section ??).
...
22 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
24 CHAPTER 17 • SEQUENCE LABELING FOR PARTS OF SPEECH AND NAMED ENTITIES
...
**17.7** Develop a set of regular expressions to recognize the character shape features described on page 17.
...
Bada, M., M. Eckert, D. Evans, K. Garcia, K. Shipley, D. Sit- nikov, W. A. Baumgartner, K. B. Cohen, K. Verspoor, J. A. Blake, and L. E. Hunter. 2012. Concept annotation in the craft corpus. *BMC bioinformatics*, 13(1):161.
...
Broschart, J. 1997. Why Tongan does it differently. *Linguis- tic Typology*, 1:123–165.
Charniak, E., C. Hendrickson, N. Jacobson, and M. Perkowitz. 1993. Equations for part-of-speech tag- ging. AAAI.
Chiticariu, L., M. Danilevsky, Y. Li, F. Reiss, and H. Zhu. 2018. SystemT: Declarative text understanding for enter- prise. NAACL HLT, volume 3.
Chiticariu, L., Y. Li, and F. R. Reiss. 2013. Rule-Based In- formation Extraction is Dead! Long Live Rule-Based In- formation Extraction Systems! EMNLP.
...
Garside, R. 1987. The CLAWS word-tagging system. In R. Garside, G. Leech, and G. Sampson, eds, *The Compu- tational Analysis of English*, 30–41. Longman.
Garside, R., G. Leech, and A. McEnery. 1997. *Corpus An- notation*. Longman.
...
Joshi, A. K. and P. Hopely. 1999. A parser from antiquity. In A. Kornai, ed., *Extended Finite State Models of Lan- guage*, 6–15. Cambridge University Press.
...
Karttunen, L. 1999. Comments on Joshi. In A. Kornai, ed., *Extended Finite State Models of Language*, 16–18. Cam- bridge University Press.
Klein, S. and R. F. Simmons. 1963. A computational ap- proach to grammatical coding of English words. *Journal of the ACM*, 10(3):334–347.
Kupiec, J. 1992. Robust part-of-speech tagging using a hid- den Markov model. *Computer Speech and Language*, 6:225–242.
Lafferty, J. D., A. McCallum, and F. C. N. Pereira. 2001. Conditional random fields: Probabilistic models for seg- menting and labeling sequence data. ICML.
...
de Marneffe, M.-C., C. D. Manning, J. Nivre, and D. Zeman. 2021. Universal Dependencies. *Computational Linguis- tics*, 47(2):255–308.
...
McCallum, A., D. Freitag, and F. C. N. Pereira. 2000. Max- imum entropy Markov models for information extraction and segmentation. ICML.
...
Sampson, G. 1987. Alternative grammatical coding systems. In R. Garside, G. Leech, and G. Sampson, eds, *The Com- putational Analysis of English*, 165–183. Longman.
Schütze, H. and Y. Singer. 1994. Part-of-speech tagging us- ing a variable memory Markov model. ACL.
Søgaard, A. 2010. Simple semi-supervised training of part- of-speech taggers. ACL.
Stolz, W. S., P. H. Tannenbaum, and F. V. Carstensen. 1965. A stochastic approach to the grammatical coding of En- glish. *CACM*, 8(6):399–405.
...
Weischedel, R., M. Meteer, R. Schwartz, L. A. Ramshaw, and J. Palmucci. 1993. Coping with ambiguity and un- known words through probabilistic models. *Computa- tional Linguistics*, 19(2):359–382.
Wu, S. and M. Dredze. 2019. Beto, Bentz, Becas: The sur- prising cross-lingual effectiveness of BERT. EMNLP.
