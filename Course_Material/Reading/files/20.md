...
[IMAGE: Diagram showing 17 relations used in the ACE relation extraction task, categorized under general groups: PERSON-SOCIAL, PHYSICAL, GENERAL-AFFILIATION, PART-WHOLE, and ARTIFACT.]
...
| :--- | :--- |
| Carey will succeed Cathleen P. Black, who held the position for 15 years and will take on a new role as chairwoman of Hearst Magazines, the company said. | Types: PERSON/TITLE. Relation: per:title |
...
$$
NP_0 \text{ such as } NP_1\{, NP_2\dots, (\text{and}|\text{or}) NP_i\}, i \ge 1 \tag{20.2}
$$

implies the following semantics

$$
NP_i, i \ge 1, \text{hyponym}(NP_i, NP_0) \tag{20.3}
$$

allowing us to infer
$$
\text{hyponym}(\text{Gelidium}, \text{red algae}) \tag{20.4}
$$
...
| Pattern | Example Patterns Matched |
| :--- | :--- |
| NP $\{, NP\}^* \{,\} (\text{and}|\text{or}) \text{other } NP_H$ | temples, treasuries, and other important civic buildings |
| $NP_H \text{ such as } \{NP,\}^* \{(\text{or}|\text{and})\} NP$ | red algae such as Gelidium |
| such $NP_H \text{ as } \{NP,\}^* \{(\text{or}|\text{and})\} NP$ | such authors as Herrick, Goldsmith, and Shakespeare |
| $NP_H \{,\} \text{ including } \{NP,\}^* \{(\text{or}|\text{and})\} NP$ | common-law countries, including Canada and England |
| $NP_H \{,\} \text{ especially } \{NP\}^* \{(\text{or}|\text{and})\} NP$ | European countries, especially France, England, and Spain |
...
*   Dependency-tree paths
    *$Airlines \xrightarrow{subj} matched \xleftarrow{comp} said \xrightarrow{subj} Wagner$*
...
[IMAGE: Diagram showing relation extraction as a linear layer on top of an encoder (BERT). The input sentence shows subject and object entities replaced by NER tags: "[CLS] [SUBJ\_PERSON] was born in [OBJ\_LOC], Michigan".]
...
*   $\text{NPH like NP}$ (*Many hormones like leptin...*)
*   $\text{NPH called NP}$ (*...using a markup language called XHTML*)
*   $\text{NP is a NPH}$ (*Ruby is a programming language...*)
*   $\text{NP, a NPH}$ (*IBM, a company with a long...*)
...
$$
V | VP | VW^*P \\
V = \text{verb particle}? \text{ adv}? \\
W = (\text{noun} | \text{adj} | \text{adv} | \text{pron} | \text{det} ) \\
P = (\text{prep} | \text{particle} | \text{infinitive “to”})
$$
...
| Features for REVERB Confidence Classifier |
| :--- |
...
$$
P = \frac{\text{# of correctly extracted relation tuples in the sample}}{\text{total # of extracted relation tuples in the sample.}} \tag{20.14}
$$
...
**reporting events**
Various versions of the event extraction task exist, depending on the goal. For example in the TempEval shared tasks (Verhagen et al. 2009) the goal is to extract events and **aspects** like their aspectual and temporal properties. Events are to be classified as actions, states, **reporting events** (*say, report, tell, explain*), perception events, and so on. The aspect, tense, and modality of each event also needs to be extracted. Thus for example the various *said* events in the sample text would be annotated as ($\text{class}=\text{REPORTING}, \text{tense}=\text{PAST}, \text{aspect}=\text{PERFECTIVE}$).
...
[IMAGE: Diagram illustrating the 13 Allen temporal relations between two intervals, A and B, including: A before B, A overlaps B, A meets B, A equals B, A starts B, A finishes B, and A during B.]
...
[IMAGE: Diagrams illustrating Reichenbach's approach to English tenses, showing the relative positions of Event time (E), Reference time (R), and Utterance time (U) on a timeline.]
...
*   $\text{Soaring}_{e1}$ is **included** in the fiscal first quarter$_{t58}$
*   $\text{Soaring}_{e1}$ is **before** 1989-10-26$_{t57}$
*   $\text{Soaring}_{e1}$ is **simultaneous** with the $\text{bucking}_{e3}$
*   $\text{Declining}_{e4}$ is **includes** $\text{soaring}_{e1}$
...
(20.35) [DCT:11/02/89]$_{t1}$: Pacific First Financial Corp. $\text{said}_{e2}$ shareholders $\text{approved}_{e3}$ its $\text{acquisition}_{e4}$ by Royal Trustco Ltd. of Toronto for $27 a share, or $212 million. The thrift holding company $\text{said}_{e5}$ it $\text{expects}_{e6}$ to $\text{obtain}_{e7}$ regulatory $\text{approvals}_{e8}$ and $\text{complete}_{e9}$ the $\text{transaction}_{e10}$ by $\text{year-end}_{t11}$.

[IMAGE: Graph showing temporal links (TLINKS in blue, ALINKS in red, SLINKS in green) among events ($e_1$ to $e_{11}$) and times in a text, illustrating temporal ordering (BEFORE, AFTER, SIMULTANEOUS, EVIDENTIAL, MODAL, ENDS, CULMINATES).]
...
$$
/(\text{\d}+) [-\s](\text{\$TEUnits})(\text{s})?([-\s]\text{old})?/
$$
...
$$
\text{pattern: } /(\text{\d}+)[-\s](\text{\$TEUnits})(\text{s})?([-\s]\text{old})?/ \\
\text{result: } \text{Duration}(\$1, \$2)
$$
...
Thus once the events and times have been detected, our goal next is to assert links between all the times and events: i.e. creating event-event, event-time, time-time, DCT-event, and DCT-time TimeML TLINKS. This can be done by training time relation classifiers to predict the correct TLINK between each pair of times/events, supervised by the gold labels in the TimeBank corpus with features like words/embeddings, parse paths, tense and aspect The sieve-based architecture using precision-ranked sets of classifiers, which we'll introduce in Chapter 23, is also commonly used.
...
$$
\text{NG}(\text{Company/ies}) \text{ VG}(\text{Set-up}) \text{ NG}(\text{Joint-Venture}) \text{ with } \text{NG}(\text{Company/ies}) \\
\text{VG}(\text{Produce}) \text{ NG}(\text{Product})
$$
...
**Figure 20.21** The five partial templates produced by stage 5 of FASTUS. These templates are merged in stage 6 to produce the final template shown in Fig. 20.19 on page 23.
...
### 20.1
Acronym expansion, the process of associating a phrase with an acronym, can be accomplished by a simple form of relational analysis. Develop a system based on the relation analysis approaches described in this chapter to populate a database of acronym expansions. If you focus on English **Three Letter Acronyms** (**TLAs**) you can evaluate your system's performance by comparing it to Wikipedia's TLA page.

### 20.2
Acquire the CMU seminar corpus and develop a template-filling system by using any of the techniques mentioned in Section 20.8. Analyze how well your system performs as compared with state-of-the-art results on this corpus.

### 20.3
A useful functionality in newer email and calendar applications is the ability to associate temporal expressions connected with events in email (doctor's appointments, meeting planning, party invitations, etc.) with specific calendar entries. Collect a corpus of email containing temporal expressions related to event planning. How do these expressions compare to the kinds of expressions commonly found in news text that we've been discussing in this chapter?
