...
**language models** or **LMs**, models that assign a **probability** to each possible next
word. Language models can also assign a probability to an entire sentence, telling
us that the following sequence has a much higher probability of appearing in a text:

all of a sudden I notice three guys standing on the sidewalk

than does that same set of words in a different order:
...
Kane et al. 2017**). People can use **AAC** systems if they are physically unable to
...
$$P(\text{blue}|\text{The water of Walden Pond is so beautifully}) = \frac{C(\text{The water of Walden Pond is so beautifully blue})}{C(\text{The water of Walden Pond is so beautifully})} \tag{3.2}$$
...
random variable $X_i$ taking on the value “the”, or $P(X_i = \text{“the”})$, we will use the
simplification $P(\text{the})$. We'll represent a sequence of $n$ words either as $w_1... w_n$ or
$w_{1:n}$. Thus the expression $w_{1:n-1}$ means the string $w_1, w_2, ..., w_{n-1}$, but we'll also
be using the equivalent notation $w_{<n}$, which can be read as "all the elements of $w$
from $w_1$ up to and including $w_{n-1}$". For the joint probability of each word in a se-
quence having a particular value $P(X_1 = w_1, X_2 = w_2, X_3 = w_3, ..., X_n = w_n)$ we'll
use $P(w_1, w_2, ..., w_n)$.
...
$$P(X_{1...X_n}) = P(X_1)P(X_2|X_1)P(X_3|X_{1:2})... P(X_n|X_{1:n-1})$$
...
exact probability of a word given a long sequence of preceding words, $P(w_n|w_{1:n-1})$.
...
all the previous words $P(w_n|w_{1:n-1})$ by using only the conditional probability of the
preceding word $P(w_n|w_{n-1})$. In other words, instead of computing the probability
...
a corpus, and **normalizing** the counts so that they lie between 0 and 1. For proba-
...
For example, to compute a particular bigram probability of a word $w_n$ given a
previous word $w_{n-1}$, we'll compute the count of the bigram $C(w_{n-1}w_n)$ and normal-
ize by the sum of all the bigrams that share the same first word $w_{n-1}$:
...
a given word $w_{n-1}$ must be equal to the unigram count for that word $w_{n-1}$ (the reader
...
$$P(\text{I}|\text{<s>}) = \frac{2}{3} \approx 0.67$$
$$P(\text{Sam}|\text{<s>}) = \frac{1}{3} \approx 0.33$$
$$P(\text{am}|\text{I}) = \frac{2}{3} \approx 0.67$$
$$P(\text{Sam}|\text{am}) = \frac{1}{2} = 0.5$$
$$P(\text{</s>}|\text{Sam}) = \frac{1}{2} = 0.5$$
$$P(\text{do}|\text{I}) = \frac{1}{3} \approx 0.33$$
...
word Chinese? The MLE of its probability is $\frac{400}{1000000}$ or 0.0004. Now 0.0004 is not
...
$$p_1 \times p_2 \times p_3 \times p_4 = \exp(\log p_1 + \log p_2 + \log p_3 + \log p_4) \tag{3.13}$$
...
gram models, when there is sufficient training data we use **trigram** models, which
...
we use two pseudo-words for the first trigram (i.e., $P(\text{I}|\text{<s><s>})$).
...
$N$ by taking the $N$th root. For a test set $W = w_1w_2...w_N$:

$$\text{perplexity}(W) = P(w_1w_2...w_N)^{-\frac{1}{N}}$$
$$= \frac{1}{\sqrt[N]{P(w_1w_2...w_N)}} \tag{3.14}$$

Or we can use the chain rule to expand the probability of $W$:

$$\text{perplexity}(W) = \left( \prod_{i=1}^{N} \frac{1}{P(w_i|w_1...w_{i-1})} \right)^{\frac{1}{N}} \tag{3.15}$$
...
word follows each other with equal probability $\frac{1}{3}$ (it was trained on a training set with
equal counts for the 3 colors), and a test set $T = \text{"red red red red blue"}$.
...
$$P(\text{red}) = 0.8 \quad P(\text{green}) = 0.1 \quad P(\text{blue}) = 0.1 \tag{3.20}$$
...
$$\approx 0.04096^{-\frac{1}{5}}$$
$$\approx 0.527^{-1} \approx 1.89 \tag{3.21}$$

## 3.4 Sampling sentences from a language model

[IMAGE: Figure 3.3, a visualization of the sampling distribution for words, showing cumulative probabilities on a number line for 'the', 'of', 'a', 'to', 'in', 'however', and 'polyphonic'.]
...
bodies is to **sample** from it. **Sampling** from a distribution means to choose random
...
| **4-gram** | -Fly, and will rid me these news of price. Therefore the sadness of parting, as they say, 'tis done. |
...
possible 4-grams is $V^4 \approx 7 \times 10^{17}$. Thus, once the generator has chosen the first
...
ally have some non-zero probability is called **smoothing** or **discounting**. Smoothing
...
one smoothing**). Since there are $V$ words in the vocabulary and each one was in-
...
turned into a probability like an MLE count by normalizing by $N$. To define this
count, since we are only changing the numerator in addition to adding 1 we'll also
need to multiply by a normalization factor $\frac{N}{N+V}$:

$$c^*_i = (c_i+1)\frac{N}{N+V} \tag{3.25}$$

We can now turn $c^*_i$ into a probability $P^*_i$ by normalizing by $N$.
...
ing algorithm in terms of a relative **discount** $d_i$, the ratio of the discounted counts
...
$$P_{\text{MLE}}(w_n|w_{n-1}) = \frac{C(w_{n-1}w_n)}{C(w_{n-1})} \tag{3.26}$$
...
$$P_{\text{Laplace}}(w_n|w_{n-1}) = \frac{C(w_{n-1}w_n)+1}{\sum_w (C(w_{n-1}w) + 1)} = \frac{C(w_{n-1}w_n)+1}{C(w_{n-1})+V} \tag{3.27}$$
...
paring Fig. 3.8 to the original counts in Fig. 3.1, we can see that $C(\text{want to})$ changed
from 608 to 238! We can see this in probability space as well: $P(\text{to}|\text{want})$ decreases
...
$$P_{\text{add-k}}(w_n|w_{n-1}) = \frac{C(w_{n-1}w_n)+k}{C(w_{n-1})+kV} \tag{3.29}$$
...
of zero frequency n-grams. If we are trying to compute $P(w_n|w_{n-2}w_{n-1})$ but we
have no examples of a particular trigram $w_{n-2}w_{n-1}w_n$, we can instead estimate its
probability by using the bigram probability $P(w_n|w_{n-1})$. Similarly, if we don't have
counts to compute $P(w_n|w_{n-1})$, we can look to the unigram $P(w_n)$. In other words,
...
trigram probability $P(w_n|w_{n-2}w_{n-1})$ by mixing together the unigram, bigram, and
trigram probabilities, each weighted by a $\lambda$:

$$P(w_n|w_{n-2}w_{n-1}) = \lambda_1 P(w_n)$$
$$+ \lambda_2 P(w_n|w_{n-1})$$
$$+ \lambda_3 P(w_n|w_{n-2}w_{n-1}) \tag{3.30}$$

The $\lambda$s must sum to 1, making Eq. 3.30 equivalent to a weighted average. In a
slightly more sophisticated version of linear interpolation, each $\lambda$ weight is com-
...
this bigram will be more trustworthy, so we can make the $\lambda$s for those trigrams
...
$$+ \lambda_3(w_{n-2:n-1})P(w_n|w_{n-2}w_{n-1}) \tag{3.31}$$

How are these $\lambda$ values set? Both the simple interpolation and conditional interpo-
lation $\lambda$s are learned from a **held-out corpus**. A held-out corpus is an additional
training corpus, so-called because we hold it out from the training data, that we use
to set these $\lambda$ values.$^4$ We do so by choosing the $\lambda$ values that maximize the likeli-
hood of the held-out corpus. That is, we fix the n-gram probabilities and then search
for the $\lambda$ values that—when plugged into Eq. 3.30—give us the highest probability
of the held-out set. There are various ways to find this optimal set of $\lambda$s. One way
is to use the **EM algorithm**, an iterative learning algorithm that converges on locally
optimal $\lambda$s (**Jelinek and Mercer, 1980**).

### 3.6.4 Stupid Backoff

An alternative to interpolation is **backoff**.$^3$ In a backoff model, if the n-gram we need
...
$$S(w_i|w_{i-N+1:i-1}) = \begin{cases} \frac{\text{count}(w_{i-N+1:i})}{\text{count}(w_{i-N+1:i-1})} & \text{if } \text{count}(w_{i-N+1:i}) > 0 \\ \lambda S(w_i|w_{i-N+2:i-1}) & \text{otherwise} \end{cases} \tag{3.32}$$

The backoff terminates in the unigram, which has score $S(w) = \frac{\text{count}(w)}{N}$. Brants et al.
...
predicting (words, letters, parts of speech), the set of which we'll call $\mathcal{X}$, and with a
particular probability function, call it $p(x)$, the entropy of the random variable $X$ is:

$$H(X) = - \sum_{x \in \mathcal{X}} p(x) \log_2 p(x) \tag{3.33}$$
...
probability of $\frac{1}{8}$. The entropy of the choice of horses is then

$$H(X) = - \sum_{i=1}^{i=8} \frac{1}{8} \log_2 \frac{1}{8} = - \log_2 \frac{1}{8} = 3 \text{ bits} \tag{3.35}$$
...
will be computing the entropy of some sequence of words $W = \{w_1, w_2, ..., w_n\}$.
...
sequence of words, and allow $W$ to represent the sequence of words $w_1, ..., w_n$, then
$L$'s entropy rate $H(L)$ is defined as

$$H(L) = \lim_{n \to \infty} \frac{1}{n} H(W_{1:n})$$
$$= \lim_{n \to \infty} \left( - \frac{1}{n} \sum_{w_{1:n} \in L} p(w_{1:n}) \log p(w_{1:n}) \right) \tag{3.38}$$
...
a bigram, $P_i$ is dependent only on $P_{i-1}$. So if we shift our time index by $x$, $P_{i+x}$ is
still dependent on $P_{i+x-1}$. But natural language is not stationary, since as we show
...
$$H(p,m) = \lim_{n \to \infty} - \frac{1}{n} \sum_{w_{1:n} \in L} p(w_1,..., w_n) \log m(w_1,..., w_n) \tag{3.40}$$
...
Between two models $m_1$ and $m_2$, the more accurate model will be the one with the
...
cross-entropy of a model $M = P(w_i|w_{i-N+1:i-1})$ on a sequence of words $W$ is
...
additional add-1 smoothed probabilities $P(\text{i} | \text{<s>}) = 0.19$ and $P(\text{</s>}|\text{food}) =
0.40$.
...
Using a bigram language model with add-one smoothing, what is $P(\text{Sam} |
\text{am})$? Include `<s>` and `</s>` in your counts just like any other token.
...
of the four possible 2 word sentences over the alphabet $\{\text{a,b}\}$ is 1.0, and the
sum of the probability of all possible 3 word sentences over the alphabet $\{\text{a,b}\}$
is also 1.0.
...
mating $P(w_3|w_1,w_2)$, where $w_3$ is a word which follows the trigram $(w_1,w_2)$,
in terms of various n-gram counts and $V$. Use the notation $c(w_1,w_2,w_3)$ to
denote the number of times that trigram $(w_1,w_2,w_3)$ occurs in the corpus, and
...
gram model and a maximum-likelihood unigram model with $\lambda_1 = \frac{1}{2}$ and $\lambda_2 =
\frac{1}{2}$, what is $P(\text{Sam}|\text{am})$? Include `<s>` and `</s>` in your counts just like any
other token.
...
each of the other digits 1–9. Now we see the following test set: 00000300
00. What is the unigram perplexity?
...
Bengio, Y., H. Schwenk, J.-S. Senécal, F. Morin, and J.-L.
...
des Sciences de St.-Pétersbourg), 7:153–162.
...
Nádas, A. 1984. Estimation of probabilities in the language
