...
[IMAGE: Figure 8.1, a diagram of a simple recurrent neural network (RNN) showing input $x_t$, hidden state $h_t$, and output $y_t$. A dashed line indicates the recurrent connection of $h_t$ feeding back into the hidden layer computation.]
...
works, an input vector representing the current input, $x_t$, is multiplied by a weight
...
responding output, $y_t$. In a departure from our earlier window-based approach, se-
quences are processed by presenting one item at a time to the network. We'll use
subscripts to represent time, thus $x_t$ will mean the input vector $x$ at time $t$. The key
...
compute an output $y_t$ for an input $x_t$, we need the activation value for the hidden
layer $h_t$. To calculate this, we multiply the input $x_t$ with the weight matrix **W**, and
the hidden layer from the previous time step $h_{t-1}$ with the weight matrix **U**. We
add these values together and pass them through a suitable activation function, $g$,
to arrive at the activation value for the current hidden layer, $h_t$. Once we have the

---

[IMAGE: Figure 8.2, a diagram showing the hidden layer computation of an RNN as a feedforward network. The previous hidden state $h_{t-1}$ is multiplied by U, added to the input $x_t$ multiplied by W, passed through activation $g$ to create $h_t$, and then multiplied by V to create output $y_t$.]

**Figure 8.2** Simple recurrent neural network illustrated as a feedforward network. The hid-
den layer $h_{t-1}$ from the prior time step is multiplied by weight matrix **U** and then added to
...
$$h_t = g(Uh_{t-1}+Wx_t) \tag{8.1}$$
$$y_t = f(Vh_t) \tag{8.2}$$

Let's refer to the input, hidden and output layer dimensions as $d_{in}$, $d_h$, and $d_{out}$
respectively. Given this, our three parameter matrices are: $\mathbf{W} \in \mathbb{R}^{d_h \times d_{in}}$, $\mathbf{U} \in \mathbb{R}^{d_h \times d_h}$,
and $\mathbf{V} \in \mathbb{R}^{d_{out} \times d_h}$.
We compute $y_t$ via a softmax computation that gives a probability distribution
over the possible output classes.

$$y_t = \text{softmax}(Vh_t) \tag{8.3}$$
...
[IMAGE: Figure 8.4, a diagram of a simple recurrent neural network unrolled in time for three time steps, showing inputs $x_1, x_2, x_3$, hidden states $h_0, h_1, h_2, h_3$, and outputs $y_1, y_2, y_3$. The weight matrices U, V, and W are explicitly shown as shared across all time steps.]
...
accruing to $h_t$, we'll need to know its influence on both the current output as well as
...
inference, computing $h_t$, $y_t$, accumulating the loss at each step in time, saving the
...
RNN language model uses $h_{t-1}$, the hidden state from the previous time step, as a
...
Section 8.1.1. The input sequence $X = [x_1; ...; x_t; ...; x_N]$ consists of a series of words
each represented as a one-hot vector of size $|V| \times 1$, and the output prediction, $y$, is a
...
[IMAGE: Figure 8.5, Simplified sketch of two LM architectures: (a) Feedforward Neural Language Model with fixed context input $e_{t-2}, e_{t-1}, e_t$ to matrix W; (b) RNN Language Model where input $e_t$ is combined with previous hidden state $h_{t-1}$ to generate $h_t$ and output $\hat{y}_t$.]
...
$h_{t-1}$ summarizes the prior context.

is, at time $t$:

$$e_t = Ex_t \tag{8.4}$$
$$h_t = g(Uh_{t-1}+We_t) \tag{8.5}$$
$$\hat{y}_t = \text{softmax}(Vh_t) \tag{8.6}$$
...
mension $d_e$ and the hidden dimension $d_h$ are the same. So we'll just call both of
these the **model dimension** $d$. So the embedding matrix $\mathbf{E}$ is of shape $[d \times |V|]$, and
$x_t$ is a one-hot vector of shape $[|V| \times 1]$. The product $e_t$ is thus of shape $[d \times 1]$. **W**
and **U** are of shape $[d \times d]$, so $h_t$ is also of shape $[d \times 1]$. **V** is of shape $[|V| \times d]$,
so the result of $Vh_t$ is a vector of shape $[|V| \times 1]$. This vector can be thought of as
a set of scores over the vocabulary given the evidence provided in $h_t$. Passing these
...
by $\hat{y}_t[k]$, the $k$th component of $\hat{y}_t$:

$$P(w_{t+1} = k|w_1,...,w_t) = \hat{y}_t[k] \tag{8.7}$$

The probability of an entire sequence is just the product of the probabilities of each
item in the sequence, where we'll use $\hat{y}_i[w_i]$ to mean the probability of the true word
$w_i$ at time step $i$.

$$P(W_{1:n}) = \prod_{i=1}^{n} P(w_i|w_{1:i-1}) \tag{8.8}$$
$$= \prod_{i=1}^{n} \hat{y}_i[w_i] \tag{8.9}$$
...
[IMAGE: Figure 8.6, a diagram illustrating RNN training as a language model, unrolled for several steps. Inputs e (embeddings of 'So', 'long', 'and', 'thanks', 'for') at each step are used to predict the Next word (y, which are the true labels 'long', 'and', 'thanks', 'for', 'all'). Loss is computed using cross-entropy LCE for each output $\hat{y}$.]
...
$$L_{\text{CE}} = - \sum_{w \in V} y_t[w] \log \hat{y}_t[w] \tag{8.10}$$

In the case of language modeling, the correct distribution $y_t$ comes from knowing the
...
$$L_{\text{CE}}(\hat{y}_t,y_t) = -\log \hat{y}_t [w_{t+1}] \tag{8.11}$$

Thus at each word position $t$ of the input, the model takes as input the the correct
word $w_t$ together with $h_{t-1}$, encoding information from the preceding $w_{1:t-1}$, and
uses them to compute a probability distribution over possible next words so as to
compute the model's loss for the next token $w_{t+1}$. Then we move to the next word,
we ignore what the model predicted for the next word and instead use the correct
word $w_{t+1}$ along with the prior history encoded to estimate the probability of token
$w_{t+2}$. This idea that we always give the model the correct history sequence to predict
...
trix **E** has shape $[d \times |V|]$. And the final layer matrix **V** provides a way to score
the likelihood of each word in the vocabulary given the evidence present in the final
hidden layer of the network through the calculation of $Vh_t$. **V** is of shape $[|V| \times d]$.
...
we dispense with **V** and use **E** at the start of the computation and $\mathbf{E}^T$ (because the
...
$$e_t = Ex_t \tag{8.12}$$
$$h_t = g(Uh_{t-1}+We_t) \tag{8.13}$$
$$\hat{y}_t = \text{softmax}(E^T h_t) \tag{8.14}$$
...
fixed set of labels to each element of a sequence, like the part-of-speech tagging and
...
$^1$ We also do this for transformers (Chapter 9) where it's common to call $\mathbf{E}^T$ the **unembedding matrix**.

---

[IMAGE: Figure 8.7, Part-of-speech tagging as sequence labeling with a simple RNN. Inputs are word embeddings (e) for words ('Janet', 'will', 'back', 'the', 'bill'), passed through the RNN layer(s) to produce hidden states (h) and outputs (Vh) which are then passed through softmax to produce a sequence of tags ($\hat{y}$) ('NNP', 'MD', 'VB', 'DT', 'NN').]
...
We can then take the hidden layer for the last token of the text, $h_n$, to constitute a
compressed representation of the entire sequence. We can pass this representation
$h_n$ to a feedforward network that chooses a class via a softmax over the possible
classes. Fig. 8.8 illustrates this approach.

[IMAGE: Figure 8.8, Sequence classification using a simple RNN combined with a feedforward network (FFN). The RNN processes inputs $x_1$ through $x_n$ sequentially, producing final hidden state $h_n$ (the context), which is passed to the FFN, and then through softmax to output the classification result $\hat{y}$.]
...
Another option, instead of using just hidden state of the last token $h_n$ to represent
the whole sequence, is to use some sort of **pooling** function of all the hidden states
$h_i$ for each word $i$ in the sequence. For example, we can create a representation that
pools all the $n$ hidden states by taking their element-wise mean:

$$h_{\text{mean}} = \frac{1}{n} \sum_{i=1}^{n} h_i \tag{8.15}$$

Or we can take the element-wise max; the element-wise max of a set of $n$ vectors is
a new vector whose $k$th element is the max of the $k$th elements of all the $n$ vectors.
...
the RNN's hidden layer and recurrent connections are hidden within the blue block.
...
[IMAGE: Figure 8.9, Autoregressive generation with an RNN-based neural language model. The RNN is unrolled, taking the previous sampled word as input (Embedding and Input Word), and outputs a distribution (Softmax) over the next Sampled Word ('So', 'long', 'and', '?'). The initial input word is `<s>`.]
...
[IMAGE: Figure 8.10, Stacked recurrent networks. Three RNN layers (RNN 1, RNN 2, RNN 3) are stacked. The output sequence of RNN 1 ($x_{1} \dots x_n$) serves as the input sequence for RNN 2, and so on, until the final output sequence $y_{1} \dots y_n$ from RNN 3.]
...
state is a function of the inputs $x_1,...,x_t$ and represents the context of the network to
the left of the current time.

$$\vec{h}_t = \text{RNN}_{\text{forward}}(x_1,...,x_t) \tag{8.16}$$

This new notation $\vec{h}_t$ simply corresponds to the normal hidden state at time $t$, repre-
...
$$\overleftarrow{h}_t = \text{RNN}_{\text{backward}}(x_t,...x_n) \tag{8.17}$$

Here, the hidden state $\overleftarrow{h}_t$ represents all the information we have discerned about the
...
$\oplus$ to mean vector concatenation:

$$h_t = [\vec{h}_t; \overleftarrow{h}_t]$$
$$= \vec{h}_t \oplus \overleftarrow{h}_t \tag{8.18}$$
...
[IMAGE: Figure 8.11, a diagram of a bidirectional RNN. RNN 1 processes the sequence $x_1 \dots x_n$ forward, and RNN 2 processes it backward. The hidden states from both RNNs at each time step ($x_i$) are concatenated to produce the final `concatenated outputs` ($y_1 \dots y_n$).]
...
[IMAGE: Figure 8.12, a diagram of a bidirectional RNN for sequence classification. Inputs $x_1$ through $x_n$ are processed forward by RNN 1 and backward by RNN 2, yielding final hidden states $\vec{h}_n$ and $\overleftarrow{h}_1$ respectively. These two states are combined (concatenated, for example) and fed into an FFN and Softmax layer to produce the output classification $y$.]
...
The first gate we'll consider is the **forget gate**. The purpose of this gate is
...
quired. Element-wise multiplication of two vectors (represented by the operator $\odot$,
...
$$f_t = \sigma(U_f h_{t-1}+W_f x_t) \tag{8.20}$$
$$k_t = c_{t-1} \odot f_t \tag{8.21}$$
...
$$g_t = \text{tanh}(U_g h_{t-1}+W_g x_t) \tag{8.22}$$

Next, we generate the mask for the **add gate** to select the information to add to the
current context.

$$i_t = \sigma(U_i h_{t-1}+W_i x_t) \tag{8.23}$$
$$j_t = g_t \odot i_t \tag{8.24}$$
...
$$o_t = \sigma(U_o h_{t-1}+W_o x_t) \tag{8.26}$$
$$h_t = o_t \odot \text{tanh}(c_t) \tag{8.27}$$

Fig. 8.13 illustrates the complete computation for a single LSTM unit. Given the

---

[IMAGE: Figure 8.13, a computation graph diagram of a single LSTM unit. Inputs are $x_t$, $h_{t-1}$, and $c_{t-1}$. Outputs are $h_t$ and $c_t$. Internal gates ($f_t, i_t, o_t$) and intermediate computations ($g_t, j_t, k_t$) are shown using $\sigma$ (sigmoid), tanh, pointwise multiplication ($\odot$), and addition (+).]

**Figure 8.13** A single LSTM unit displayed as a computation graph. The inputs to each unit consists of the
current input, $x_t$, the previous hidden state, $h_{t-1}$, and the previous context, $c_{t-1}$. The outputs are a new hidden
state, $h_t$ and an updated context, $c_t$.
...
It is the hidden state, $h_t$, that provides the output for the LSTM at each time step.
This output can be used as the input to subsequent layers in a stacked RNN, or at the
final layer of a network $h_t$ can be used to provide the final output of the LSTM.
...
[IMAGE: Figure 8.14, diagrams illustrating the basic neural units: (a) Feedforward unit with input $x_t$ and output $h_t$, involving summation ($\Sigma$) and activation ($g$). (b) Simple Recurrent Network (SRN) unit with inputs $x_t$ and $h_{t-1}$ and output $h_t$. (c) Long Short-Term Memory (LSTM) Unit with inputs $x_t, h_{t-1}, c_{t-1}$ and outputs $h_t, c_t$.]
...
[IMAGE: Figure 8.15, Four architectures for NLP tasks: (a) sequence labeling, (b) sequence classification, (c) language modeling, and (d) encoder-decoder, illustrated with RNN blocks.]

**Figure 8.15** Four architectures for NLP tasks. In sequence labeling (POS or named entity tagging) we map
each input token $x_i$ to an output token $y_i$. In sequence classification we map the entire input sequence to a single
...
[IMAGE: Figure 8.16, The encoder-decoder architecture. The Encoder block takes input $x_1 \dots x_n$ and outputs a Context vector. The Decoder block takes the Context vector as input and generates an output sequence $y_1 \dots y_m$.]
...
1. An **encoder** that accepts an input sequence, $x_{1:n}$, and generates a correspond-

---

ing sequence of contextualized representations, $h_{1:n}$. LSTMs, convolutional
networks, and transformers can all be employed as encoders.
2. A **context vector**, $c$, which is a function of $h_{1:n}$, and conveys the essence of
the input to the decoder.
3. A **decoder**, which accepts $c$ as input and generates an arbitrary length se-
quence of hidden states $h_{1:m}$, from which a corresponding sequence of output
...
$$p(y) = p(y_1)p(y_2|y_1)p(y_3|y_1, y_2) ... p(y_m|y_1, ..., y_{m-1}) \tag{8.28}$$
...
set of possible vocabulary items, then at time $t$ the output $\hat{y}_t$, and hidden state $h_t$ are
computed as:

$$h_t = g(h_{t-1},x_t) \tag{8.29}$$
$$\hat{y}_t = \text{softmax}(h_t) \tag{8.30}$$
...
$$p(y|x) = P(y_1|x)P(y_2|y_1,x)P(y_3|y_1,y_2,x)...P(y_m|y_1,...,y_{m-1},x) \tag{8.31}$$
...
[IMAGE: Figure 8.17, Training a single sentence in the basic RNN encoder-decoder approach for machine translation. The Source Text ('the green witch arrived') and Separator (`<s>`) are processed by the Encoder. The hidden state $h_n^e$ from the Encoder is passed to the Decoder, which autoregressively generates the Target Text ('llegó la bruja verde', `</s>`).]
...
[IMAGE: Figure 8.18, a more formal version of translating a sentence at inference time in the basic RNN-based encoder-decoder architecture. The final hidden state of the encoder RNN, $h_n^e$ (the context $c$) is used to initialize the first hidden state of the decoder $h_0^d$, and is also available to each subsequent decoder hidden state $h_i^d$. The decoder generates the output sequence $y_1 \dots y_m$.]

**Figure 8.18** A more formal version of translating a sentence at inference time in the basic RNN-based
encoder-decoder architecture. The final hidden state of the encoder RNN, $h_n^e$, serves as the context for the
decoder in its role as $h_0^d$ in the decoder RNN, and is also made available to each decoder hidden state.
...
$h_n^e$. This representation, also called $c$ for **context**, is then passed to the decoder.
...
would use $c$ as its prior hidden state $h_0^d$. The decoder would then autoregressively
...
$$h_i^d = g(\hat{y}_{i-1}, h_{i-1}^d, c) \tag{8.32}$$
...
that $g$ is a stand-in for some flavor of RNN and $\hat{y}_{i-1}$ is the embedding for the output
sampled from the softmax at the previous step:

$$c = h_n^e$$
$$h_0^d = c$$
$$h_i^d = g(\hat{y}_{i-1}, h_{i-1}^d, c)$$
$$\hat{y}_i = \text{softmax}(h_i^d) \tag{8.33}$$

Thus $\hat{y}_i$ is a vector of probabilities over the vocabulary, representing the probability
of each word occurring at time $t$. To generate text, we sample from this distribution
$\hat{y}_i$. For example, the greedy choice is simply to choose the most most probable word
...
estimated output $\hat{y}_t$ as the input for the next time step $x_{t+1}$. Thus the decoder will
...
from training as the next input $x_{t+1}$, rather than allowing it to rely on the (possibly
erroneous) decoder output $\hat{y}_t$. This speeds up training.

---

[IMAGE: Figure 8.19, Training the basic RNN encoder-decoder approach to machine translation. The Decoder uses gold answers ($y_1$='llegó', $y_2$='la', $y_3$='bruja', $y_4$='verde', $y_5$='</s>') as the input for the next step, instead of its own predicted output $\hat{y}$. Loss is computed at each step based on the softmax distribution $\hat{y}$ vs the gold answers. Total loss is the average cross-entropy loss per target word.]

**Figure 8.19** Training the basic RNN encoder-decoder approach to machine translation. Note that in the
decoder we usually don't propagate the model's softmax outputs $\hat{y}_t$, but use **teacher forcing** to force each input
to the correct gold value for training. We compute the softmax output distribution over $\hat{y}$ in the decoder in order
...
context vector is $h_n^e$, the hidden state of the last ($n$th) time step of the source text.
...
[IMAGE: Figure 8.20, Encoder-decoder diagram showing the context $c$ originating only from the final hidden state of the Encoder, highlighting the bottleneck problem.]
...
is, $c = f(h_1^e, ... h_n^e)$. Because the number of hidden states varies with the size of
...
decoding.
This context vector, $c_i$, is generated anew with each decoding step $i$ and takes
...
$$h_i^d = g(\hat{y}_{i-1}, h_{i-1}^d, c_i) \tag{8.34}$$

[IMAGE: Figure 8.21, Diagram of the attention mechanism showing that the context vector $c_i$ for each decoder hidden state $h_i^d$ is generated as a function of all encoder hidden states ($h_1^e, h_2^e, h_3^e, \dots$).]
...
The first step in computing $c_i$ is to compute how much to focus on each encoder
state, how **relevant** each encoder state is to the decoder state captured in $h_{i-1}^d$. We
capture relevance by computing— at each step $i$ during decoding—a $score(\mathbf{h}_{i-1}^d, h_j^e)$
...
$$\text{score}(\mathbf{h}_{i-1}^d, h_j^e) = \mathbf{h}_{i-1}^d \cdot h_j^e \tag{8.35}$$
...
vector of weights, $\alpha_{ij}$, that tells us the proportional relevance of each encoder hidden
state $j$ to the prior hidden decoder state, $h_{i-1}^d$.

$$\alpha_{ij} = \text{softmax}(\text{score}(h_{i-1}^d, h_j^e))$$
$$= \frac{\exp(\text{score}(h_{i-1}^d, h_j^e))}{\sum_k \exp(\text{score}(h_{i-1}^d, h_k^e))} \tag{8.36}$$

Finally, given the distribution in $\alpha$, we can compute a fixed-length context vector for
the current decoder state by taking a weighted average over all the encoder hidden
states.

$$c_i = \sum_j \alpha_{ij} h_j^e \tag{8.37}$$
...
[IMAGE: Figure 8.22, A sketch of the encoder-decoder network with attention, focusing on the computation of $c_i$. The attention weights $\alpha_{ij}$ are calculated between the prior decoder hidden state $h_{i-1}^d$ and all encoder hidden states $h_j^e$ (using dot product), and then used in a weighted sum to compute the context vector $c_i$, which is then used as input to compute the current decoder hidden state $h_i^d$.]

**Figure 8.22** A sketch of the encoder-decoder network with attention, focusing on the computation of $c_i$. The
context value $c_i$ is one of the inputs to the computation of $h_i^d$. It is computed by taking the weighted sum of all
the encoder hidden states, each weighted by their dot product with the prior decoder hidden state $h_{i-1}^d$.
...
by parameterizing the score with its own set of weights, $W_s$.

$$\text{score}(h_{i-1}^d, h_j^e) = h_{i-1}^d W_s h_j^e$$

The weights $W_s$, which are then trained during normal end-to-end training, give the
...
networks. ArXiv.
Graves, A., S. Fernández, M. Liwicki, H. Bunke, and
...
S. Amir, L. Marujo, and T. Luís. 2015. Finding function
...
ICLR 2013.
Mikolov, T., M. Karafiát, L. Burget, J. Černockỳ, and
...
unified relational semantic representation. Int. J. Seman-
