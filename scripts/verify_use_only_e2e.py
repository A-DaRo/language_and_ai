...
# =============================================================================
# Single-Label Mode Verification
# =============================================================================
...
def run_phase_d(
    ...
    try:
        ...
        from neuro_stylometry.phase_d_pipeline import PhaseDPipeline
        from neuro_stylometry.config import load_phase_d_config
        
        print_substep("Loading Phase D configuration...")
        config = load_phase_d_config(mode=mode)
        
        # Update config to use Phase A outputs
        config["input_dataset"] = str(phase_a_output / "clean_dataset.arrow")
        config["projection_matrix"] = str(phase_a_output / "projection_matrix.pt")
        ...
        print_substep("Running Phase D pipeline...")
        pipeline = PhaseDPipeline(config=config)
        
        start_time = time.time()
        phase_d_result = pipeline.run(
        ...
        result.info["elapsed_seconds"] = elapsed
        print(f"    Phase D completed in {elapsed:.1f}s")
        
        return result
        
    except ImportError as e:
        result.warnings.append(f"Phase D not available: {e}")
        print(f"    ⚠ Phase D skipped: {e}")
        return result
    except Exception as e:
        result.passed = False
        result.errors.append(f"Phase D execution failed: {e}")
        logger.exception("Phase D execution error")
        return result
...
Examples:
    ...
    total_steps = 5 if not args.skip_phase_d else 4
