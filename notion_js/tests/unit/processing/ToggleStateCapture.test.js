...
describe('ToggleStateCapture', () => {
  ...
  describe('Content Locator Heuristics', () => {
    ...
    test('Strategy 1: finds sibling content by data-content-for attribute', async () => {
      const mockPage = createMockPage({
        evaluateResult: '<div class="toggle-content">Real content here</div>'
      });
      
      const mockToggle = { handle: {}, click: jest.fn() };
      const result = await capture._getToggleContentHtml(null, mockPage, mockToggle);
      
      expect(result).toContain('toggle-content');
    });

    test('returns empty string when no content found', async () => {
      const mockPage = createMockPage({
        evaluateResult: ''
      });
  ...
  describe('_waitForContentChange()', () => {
    test('resolves when content changes', async () => {
      ...
      await capture._waitForContentChange(mockFacade, mockPage, mockToggle, '');
    ...
    test('times out after contentWait ms', async () => {
      ...
      const startTime = Date.now();
      await capture._waitForContentChange({}, mockPage, { handle: {} }, '');
      const elapsed = Date.now() - startTime;
